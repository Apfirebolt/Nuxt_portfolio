  ...
  <NuxtLayout name="default">
    <div
      ...
      <main class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <article v-else-if="project && project.images"
          ...
          <div class="flex flex-wrap items-center justify-between gap-4 border-b border-secondary-dark/15 pb-6">
            ...
            <time v-if="project.date_posted" class="text-xs font-medium text-primary-dark/60">
              Published on {{ new Date(project.date_posted).toLocaleDateString('en-US', {
                year: 'numeric', month:
                  'long', day: 'numeric' }) }}
            </time>
          </div>
          ...
          <div class="mt-8">
            ...
            <div class="mt-6 flex flex-wrap gap-4 border-y border-secondary-dark/15 py-4">
              <div v-if="project.technology" class="flex flex-wrap items-center gap-2">
                ...
                <div class="flex flex-wrap gap-1.5">
                  <span v-for="(tech, index) in project.technology.split(',')" :key="index"
            ...
            <div v-if="isClient && project.description"
          ...
          <div class="mt-12">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              <div v-for="(image, index) in project.images" :key="index" @click="openLightbox(index)"
                ...
                <img :src="getImageUrl(image.image)" :alt="image.caption || project.title"
                  class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
                  loading="lazy" />

                <div
                  class="absolute inset-0 bg-gradient-to-t from-primary-dark/70 via-transparent to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />

                <div
                  class="absolute bottom-0 left-0 right-0 p-4 text-primary opacity-0 transition-opacity duration-300 group-hover:opacity-100 flex items-center justify-between">
      ...
      <Teleport to="body">
        <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0"
          ...
          <div v-if="showLightbox && isClient && project?.images?.length"
            ...
            <button v-if="project.images.length > 1" type="button" @click.stop="prevImage"
            ...
            <div class="flex max-h-[85vh] max-w-5xl flex-col items-center justify-center">
              <img :src="getImageUrl(project.images[currentImageIndex].image)"
              ...
              <p v-if="project.images[currentImageIndex].caption"
                class="mt-3 text-center text-xs font-medium text-primary/80">
                {{ project.images[currentImageIndex].caption }}
              </p>
            </div>

            <button v-if="project.images.length > 1" type="button" @click.stop="nextImage"
            ...
            <div
              class="absolute bottom-6 left-1/2 -translate-x-1/2 rounded-full border border-primary/20 bg-primary-dark/80 px-4 py-1.5 text-xs font-semibold text-primary backdrop-blur-sm">
