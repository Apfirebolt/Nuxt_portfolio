<template>
  <NuxtLayout name="default">
    <div
      class="min-h-screen bg-primary text-primary-dark antialiased selection:bg-secondary-dark selection:text-primary">
      <section class="relative overflow-hidden border-b border-secondary-dark/15 bg-secondary/30 py-16 sm:py-24">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div class="hero grid grid-cols-1 items-center gap-12 lg:grid-cols-12 lg:gap-10">
            <div class="flex flex-col justify-center lg:col-span-7">
              <div
                class="inline-flex items-center gap-2 self-start rounded-full border border-secondary-dark/30 bg-primary px-3.5 py-1 text-xs font-semibold uppercase tracking-wider text-secondary-dark shadow-sm">
                <span class="h-2 w-2 rounded-full bg-secondary-dark animate-pulse"></span>
                Articles, Architecture &amp; Insights
              </div>

              <h1 class="mt-6 text-4xl font-black tracking-tight text-primary-dark sm:text-5xl lg:text-6xl">
                Engineering <span class="text-secondary-dark">Blog</span>
              </h1>

              <p class="mt-6 text-lg leading-relaxed text-primary-dark/80">
                Detailed write-ups, deep dives into distributed backend systems,
                system design patterns, and notes from continuous exploration.
              </p>

              <div class="mt-8 flex flex-wrap gap-3 text-xs font-semibold text-primary-dark/90">
                <span class="rounded-lg border border-secondary-dark/20 bg-primary px-3 py-1.5 shadow-sm">💻 Backend
                  &amp; Architecture</span>
                <span class="rounded-lg border border-secondary-dark/20 bg-primary px-3 py-1.5 shadow-sm">🚀 Full-Stack
                  Development</span>
                <span class="rounded-lg border border-secondary-dark/20 bg-primary px-3 py-1.5 shadow-sm">🗺️
                  Reflections &amp; Travel</span>
              </div>
            </div>

            <div class="relative flex justify-center lg:col-span-5">
              <div
                class="relative aspect-4/3 w-full max-w-md overflow-hidden rounded-2xl border border-secondary-dark/30 bg-secondary shadow-2xl lg:max-w-lg">
                <img class="h-full w-full object-cover object-center transition-transform duration-500 hover:scale-105"
                  src="https://softgenie.org/media/images/generic/my_blog.jpeg" alt="Engineering Blog Showcase"
                  loading="lazy" />
              </div>
            </div>
          </div>
      ...
      <main class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        <div v-else>
          <div v-if="blogs.results && blogs.results.length" class="space-y-12">
            <article v-for="blog in blogs.results" :key="blog.id"
              ...
              <div class="flex flex-wrap items-center justify-between gap-4 border-b border-secondary-dark/15 pb-4">
                <div class="flex flex-wrap items-center gap-2">
                  <span v-for="tag in blog.tags" :key="tag.name || tag"
              ...
              <div class="mt-5">
                ...
                <div class="prose prose-stone mt-3 line-clamp-3 max-w-none text-sm leading-relaxed text-primary-dark/80"
              ...
              <div v-if="blog.images && blog.images.length"
                ...
                <div v-for="item in blog.images" :key="item.id"
                  ...
                  <div class="relative aspect-16/10 w-full overflow-hidden bg-secondary">
                    <img :src="getFullImageUrl(item.image)" :alt="item.caption || blog.title"
                      class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
                      loading="lazy" />
                    <button type="button" @click="viewImageInFullSize(item.image)"
              ...
              <div class="mt-8 flex items-center justify-end border-t border-secondary-dark/15 pt-5">
                <button type="button" @click="goToDetail(blog)"
          ...
          <div v-else
            ...
            <h3 class="mt-4 text-base font-bold text-primary-dark">
              No Articles Published Yet
            </h3>
            <p class="mt-1 text-xs text-primary-dark/60">
              New posts and technical breakdowns will be published soon.
            </p>
          </div>
        </div>
      </main>
...
useHead({
  title: "Blog | APGIIIT.com - Software & Engineering Articles",
});

definePageMeta({
  layout: false,
  title: "Blog - Amit Prafulla",
  description:
    "Articles and deep dives on full-stack web development, distributed architecture, and software design.",
});
